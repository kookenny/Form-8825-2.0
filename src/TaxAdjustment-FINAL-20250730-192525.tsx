...
function TaxAdjustment({ 
  ...
  // Base Form 8825 options
  const baseForm8825Options = [
    '8825.02 Gross rents',
  ...
  // Combine base options with custom tax groups for Form 8825
  const getForm8825Options = () => {
    const allOptions = [...baseForm8825Options]
    ...
    setScheduleData(prev => 
      prev.map((item, i) => 
        i === index ? { 
          ...
          group: '', // Clear group when schedule changes
          property: newSchedule !== "8825" ? '' : item.property // Clear property when schedule is not 8825
        } : item
      )
    )
  }
  ...
  const handlePropertyChange = (index: number, newProperty: string) => {
    // Only allow property changes when schedule is "8825"
    setScheduleData(prev => 
      prev.map((item, i) => 
        i === index && item.schedule === "8825" ? { ...item, property: newProperty } : item
      )
    )
  }
  ...
  const getPropertyOptions = () => {
    ...
    for (let i = 1; i <= propertyCount; i++) {
      ...
      console.log(`TaxAdjustment - Property ${i}:`, property, 'propertyId:', propertyId, 'propertyName:', propertyName)
    ...
    <div className="tax-adjustment-container">
      ...
      <div className="tax-adjustment-content">
        ...
        <div className="tax-adjustment-buttons">
          <button className="tax-adjustment-btn" onClick={onBack}>Engagement Setup</button>
                  ...
                  {showPropertyColumn && (
                    <td className="property-cell">
                      <select 
                        ...
                        disabled={item.schedule !== "8825"}
                        style={{
                          opacity: item.schedule !== "8825" ? 0.5 : 1,
                          cursor: item.schedule !== "8825" ? 'not-allowed' : 'pointer'
                        }}
                      >
