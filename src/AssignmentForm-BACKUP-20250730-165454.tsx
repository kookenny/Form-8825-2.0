...
interface AssignmentFormProps {
  ...
  setAccountData: React.Dispatch<React.SetStateAction<Array<{
    number: string
    name: string
    amount2023: string
    amount2022: string
...
function AssignmentForm({ 
  ...
  console.log('AssignmentForm - propertyDetails:', propertyDetails, 'numberOfProperties:', numberOfProperties)
  ...
  console.log('AssignmentForm - propertyCount:', propertyCount)
  
  for (let i = 1; i <= propertyCount; i++) {
    const property = propertyDetails?.[i - 1]
    const propertyId = property?.propertyId
    const propertyName = property?.propertyName
    console.log(`AssignmentForm - Property ${i}:`, property, 'propertyId:', propertyId, 'propertyName:', propertyName)
    
    let displayText = `Property ${i}`
    if (propertyId) {
      displayText = `${i}-${propertyId}`
      if (propertyName && propertyName.trim()) {
        displayText += ` (${propertyName})`
      }
    }
    
    propertyOptions.push(displayText)
  }
  
  console.log('AssignmentForm - final propertyOptions:', propertyOptions)
  
  // Base tax group options
  const baseTaxGroupOptions = [
  ...
  // Combine base options with custom tax groups
  const getAllTaxGroupOptions = () => {
    const allOptions = [...baseTaxGroupOptions]
    
    // Group custom tax groups by their parent
    const customGroupsByParent: { [key: string]: typeof customTaxGroups } = {}
    customTaxGroups.forEach(group => {
      if (!customGroupsByParent[group.parentCode]) {
        customGroupsByParent[group.parentCode] = []
      }
      customGroupsByParent[group.parentCode].push(group)
    })
    
    // Insert custom options after their parent groups
    Object.entries(customGroupsByParent).forEach(([parentCode, groups]) => {
      const parentIndex = allOptions.findIndex(option => option.startsWith(parentCode + ' '))
      
      if (parentIndex !== -1) {
        // Sort custom groups by code for consistent ordering
        const sortedGroups = groups.sort((a, b) => a.code.localeCompare(b.code))
        
        // Insert each custom group after its parent (in reverse order to maintain positions)
        sortedGroups.reverse().forEach(group => {
          allOptions.splice(parentIndex + 1, 0, `  ${group.code} ${group.name}`) // Add indentation for visual hierarchy
        })
      }
    })
  ...
  const taxGroupOptions = getAllTaxGroupOptions()
    ...
    setAccountData(prev => 
      prev.map((account, i) => 
        i === index ? { 
          ...
          // Clear property if tax group doesn't start with 8825 (trim whitespace for custom groups)
          property: newTaxGroup.trim().startsWith('8825') ? account.property : ''
        } : account
      )
    )
  }
    ...
    <div className="assignment-container">
      ...
      <div className="assignment-content">
        ...
        <div className="assignment-buttons">
          <button className="assignment-btn" onClick={onBack}>Engagement Setup</button>
                  ...
                  {showPropertyColumn && (
                    <td className="property-cell" style={{ width: columnWidths.property }}>
                      <select 
                        ...
                        onClick={(e) => e.stopPropagation()}
                        disabled={!account.taxGroup.trim().startsWith('8825')}
                      >
